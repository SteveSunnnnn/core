...
void VulkanDesktopBackend::create_hdr_targets() {
    ...
    // hdr_sampler_ is created once in create_live_validation_renderer and
...
void VulkanDesktopBackend::create_live_validation_renderer() {
    ...
    // Terrain and ocean declare a single 32-byte block holding the map viewport
    // followed by per-pass parameters, and validation requires the whole block
    // to be contained by a range matching that stage. One range covering both
    // stages satisfies that; shaders that declare less (living) or nothing are
    // unaffected by a range they do not use.
    ...
    fullscreen_range.size = sizeof(float) * 8u;
    ...
    // The terrain FBM octave counts are baked in as specialization constants.
    // Driving them from a uniform instead stopped the driver unrolling the
    // octave loop and cost roughly 7% of the frame, so the tier is resolved
    // once here and the shader compiles to straight-line code.
    const std::int32_t terrain_octave_values[2]{
        static_cast<std::int32_t>(settings_.terrain_octaves),
        static_cast<std::int32_t>(settings_.terrain_detail_octaves)};
    const VkSpecializationMapEntry terrain_octave_entries[2]{
        {0u, 0u, sizeof(std::int32_t)},
        {1u, sizeof(std::int32_t), sizeof(std::int32_t)}};
    const VkSpecializationInfo terrain_specialization{
        2u, terrain_octave_entries, sizeof(terrain_octave_values), terrain_octave_values};

    // Fullscreen passes do not depth-test: they emit at depth 0 and would
    // otherwise occlude the real 3D geometry drawn afterwards.
    terrain_pipeline_ = create_pipeline(fullscreen, terrain, fullscreen_layout_, false, nullptr,
